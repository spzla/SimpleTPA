...
public class TPRejectCommand  implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender,
        ...
        if ( strings.length == 0 || strings[0].isEmpty() ) {
            request = SimpleTPA.INSTANCE.findTo(sender);
            ...
            if (from.equals(sender)) {
                sender.sendMessage("nie mozesz odrzucic tpa sam do siebie");
                return true;
            }
            ...
            if ( request == null ) {
                sender.sendMessage(String.format("Nie masz oczekujacych prosb o teleportacje od gracza %s", from.getName()));
                return true;
            }
        }

        SimpleTPA.INSTANCE.rejectRequest(request);
